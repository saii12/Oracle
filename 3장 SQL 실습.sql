...
// 직업별로 중복제거해서 DEPTNO 보여줌
SELECT DISTINCT JOB, DEPTNO FROM EMP;
// COMM 칼럼 NULL이 되면 더하기 연산 안되고 더한 값도 NULL로 나오네, 그럼 테이블 생성시 DEFAULT 0으로 하지..
...
SELECT * FROM EMP WHERE COMM = NULL; // COMM = NULL 조회 안됨
SELECT * FROM EMP WHERE COMM IS NULL;
SELECT * FROM EMP WHERE COMM IS NOT NULL;

-- 실습하기 2-2. 집합 연산자를 이용한 데이터 조회 / 합집합
...
// UPPER, LOWER : 문자 대소문자 / INITCAP : 첫글자 대문자
SELECT ENAME, UPPER(ENAME), LOWER(ENAME), INITCAP(ENAME) FROM EMP;

// SUBSTR : 문자 자르기 / 몇번째 문자부터 몇 개
SELECT JOB, SUBSTR(JOB, 1, 2), SUBSTR(JOB, 3, 2), SUBSTR(JOB, 5) FROM EMP;

// INSTR: 문자 위치 / -1은 뒤에서부터 위치 찾기
SELECT
    INSTR('HELLO ORACLE!', 'L') AS INSTR_1,
    INSTR('HELLO ORACLE!', 'L', -1) AS INSTR_2
FROM DUAL;

// REPLACE : 문자 교체 / DUAL은 임시테이블 
...
// NVL, NVL2 : NULL 체크 함수 / NVL(COMM, 0) COMM이 NULL이면 0으로 표시
SELECT 
    EMPNO,
    ENAME,
    SAL,
